  ...
  if (questions.length === 0) {
    return (
      <div className="max-w-md mx-auto space-y-6 text-center py-12">
        ...
        <p className="text-sm text-slate-400">This quiz exists in the database, but it does not contain any questions yet.</p>
        <Link to={`/lesson/${lesson.id}`} className="inline-flex px-5 py-2.5 rounded-xl bg-indigo-500 text-white font-semibold text-xs">
          Back to Lesson
        </Link>
      </div>
    )
