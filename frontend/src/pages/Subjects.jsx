...
export default function Subjects() {
      ...
      <div className="space-y-6 animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-6">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="h-64 bg-slate-800 rounded-3xl"
            ></div>
          ))}
        </div>
      </div>
  ...
  const getLessonStats = (lessonId) => {
    ...
    const highestScore = Math.max(
      ...attempts.map((a) => Number(a.quiz_score) || 0)
    )
    ...
    const highestDifficulty = [...attempts].sort((a, b) => {
      ...
      return (
        (order[String(b.difficulty || '').toUpperCase()] || 0) -
        (order[String(a.difficulty || '').toUpperCase()] || 0)
      )
    })[0]?.difficulty
  ...
  const filteredLessons = lessons.filter(
    (lesson) =>
      getSubjectFromLesson(lesson) === selectedSubject
  )
    ...
    <div className="space-y-8">
      <div className="space-y-1.5">
        ...
        <p className="text-slate-400 text-sm leading-relaxed max-w-2xl">
          Select a subject and explore topic modules. Aegis automatically
          scales assessment difficulty based on performance.
        </p>
      </div>
      ...
      {Object.keys(groupedLessons).length === 0 ? (
        <div className="glass-panel rounded-3xl p-12 border border-slate-900 shadow-md text-center py-16 space-y-4">
          ...
          <p className="text-xs text-slate-400 max-w-sm mx-auto leading-relaxed">
            No {selectedSubject} lessons were found in the database.
