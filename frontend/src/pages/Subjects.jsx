...
export default function Subjects() {
  ...
  const filteredLessons = lessons.filter(
    (lesson) => getSubjectFromLesson(lesson).toLowerCase() === selectedSubject.toLowerCase()
  )
                    ...
                    >
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-slate-500">
                          <span>{lesson.quizzes?.length || 0} difficulty levels</span>
                          {lesson.quizzes?.length > 0 && <span>&bull;</span>}
                          {lesson.quizzes?.length > 0 && <span>{lesson.quizzes.reduce((sum, q) => sum + (q.question_count || 0), 0)} questions</span>}
                        </div>
                      </div>
                      ...
                      <div className="border-t border-slate-800/60 pt-4 flex items-center justify-between">
                        ...
                        >
                          {stats?.completed ? 'Review Questions' : 'Open Lesson & Questions'}
                        </Link>
                      </div>
                    </div>
                  )
